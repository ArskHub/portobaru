...
export function HeaderNavigation() {
  ...
  const headerData = [
    ...
    { name: "Work", href: links.work },
    { name: "Arsip", href: links.archive },
    { name: "About", href: links.about },
    { name: "Contact", href: links.contact },
    { name: "CV", href: links.cv },
  ];
  ...
  return (
    <>
      ...
      >
        ...
        />
        <nav className="flex w-full flex-1 items-center justify-center overflow-y-auto">
          <ul className="mask flex flex-col items-center justify-center gap-1 px-8 py-[4vh]">
            {headerData.map((data) => (
              <li className="headerAnimate" key={data.name}>
                <Magentic
                  className={`text-[clamp(26px,_2.6vw_+_16px,_60px)] font-bold leading-[1.15] text-color${
